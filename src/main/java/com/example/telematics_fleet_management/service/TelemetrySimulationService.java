...
public class TelemetrySimulationService {
    ...
    public void generateData() {
        ...
        User driver = userRepository.findByEmail("driver@gmail.com");

        for (Vehicle vehicle : vehicles) {

            // Telemetry every 5 seconds
            telemetryRepository.save(telemetryGenerator.generate(vehicle));

            double x = Math.random();

            // 20% chance to create Trip
            if (x < 0.2 && driver != null) {
                tripRepository.save(tripGenerator.generate(vehicle, driver));
            }

            // 10% chance to create Fuel Log
            if (x < 0.1) {
                fuelLogRepository.save(fuelLogGenerator.generate(vehicle));
            }

            // 5% chance to create Service Record
            if (x < 0.05) {
                serviceRecordRepository.save(serviceGenerator.generate(vehicle));
            }

            if (x < 0.2 && driver != null) {
                DriverScore previousScore = driverScoreRepository.findTopByDriverOrderByScoreIdDesc(driver);
                driverScoreRepository.save(driverScoreGenerator.generate(driver, vehicle, previousScore));
            }
        }
        log.info("Telemetry data generated for all vehicles.");
    }
}